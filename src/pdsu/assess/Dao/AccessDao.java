...
public class AccessDao {
    ...
    public static int changeSalary(String changeType,String personid,String poldaccess,String pnewaccess){
        int flag=0;
        try{
            ...
            String sql2="insert into history values("+str[1]+",'"+changeType+"','"+poldaccess+"','"+pnewaccess+"','"+df.format(new Date())+"','"+str[0]+"',"+personId+")";
            String sql ="update  person set assess='"+pnewaccess+"' where personId="+personId;
            Statement st = JDBCUtil.getConnection();
            System.out.print(sql);
    ...
    public static String[][] getAllHistory(){
        ...
        try{
            String sql ="select * from history where histType='ÈËÔ±¿¼ºË'";
            Statement st =JDBCUtil.getConnection();
            ResultSet rs =st.executeQuery(sql);
            while (rs.next()){
